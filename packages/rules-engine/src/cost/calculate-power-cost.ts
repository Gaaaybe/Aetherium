...
export function calculatePowerCost({
  ...
  effectBases,
  modificationBases,
}: PowerCostCalculationInput): {
  success: boolean;
  error?: string;
  result?: PowerCostCalculationResult;
} {
  const custoPorEfeito: Record<string, PowerCost> = {};
  let totalPdA = 0;
  const espacosPorEfeito: number[] = [];
  const pePorEfeito: number[] = [];

  const defaultParamsPerEffect: Array<{ acao: number; alcance: number; duracao: number }> = [];
  ...
  const paramsModifierByGrade = calculateGlobalParamsModifier(
  ...
  for (const appliedEffect of effects) {
    ...
    for (const globalMod of globalModifications) {
      ...
      const { custoFixo: cfGlobal, custoPorGrau: cpgGlobal } =
        calcularCustoComConfiguracao(modBase, selectedConfigurationId);
  ...
  const getOpcao = (mod: AppliedModificationInput) =>
...
function getUniversalGradeData(grau: number): { pe: number; espacos: number } {
  return UNIVERSAL_TABLE.find((row) => row.grau === grau) ?? { pe: 0, espacos: 0 };
}

function calculateGlobalParamsModifier(
  defaultParamsPerEffect: Array<{ acao: number; alcance: number; duracao: number }>,
  selectedParams: PowerParametersInput,
): number {
  ...
  return (
    calculateParameterModifier(defaultAction, selectedParams.acao, 'acao') +
...
function calculateParameterModifier(
  defaultValue: number,
...
function calcularCustoComConfiguracao(
  modBase: ModificationBaseCatalogItem,
  configuracaoId?: string,
): { custoFixo: number; custoPorGrau: number } {
