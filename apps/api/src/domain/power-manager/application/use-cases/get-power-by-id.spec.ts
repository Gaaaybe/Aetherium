...
describe('GetPowerByIdUseCase', () => {
  ...
  beforeEach(() => {
    ...
    sut = new GetPowerByIdUseCase(powersRepository);
  });

  it('should get a power by id', async () => {
    ...
    if (result.isRight()) {
      expect(result.value.power.id.equals(power.id)).toBe(true);
      expect(result.value.power.nome).toBe('Rajada de Energia');
      expect(result.value.power.descricao).toBe('Dispara uma rajada de energia');
  ...
  it('should get the correct power when multiple powers exist', async () => {
    const effectBase = EffectBase.create({
      id: 'dano',
      nome: 'Dano',
      custoBase: 1,
    ...
    const result = await sut.execute({
      powerId: power2.id.toString(),
    });

    expect(result.isRight()).toBe(true);
    if (result.isRight()) {
      expect(result.value.power.id.equals(power2.id)).toBe(true);
      expect(result.value.power.nome).toBe('Rajada 2');
    }
  });
});
