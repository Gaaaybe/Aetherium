...
describe('Unequip Power Array (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer()).post('/users').send({
      name: 'Unequip Array User',
      email: 'unequiparray@example.com',
      password: '123456',
    });
  ...
  test('[PATCH] /characters/:id/power-arrays/:arrayId/unequip', async () => {
    const response = await request(app.getHttpServer())
      .patch(`/characters/${characterId}/power-arrays/${arrayId}/unequip`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    expect(response.statusCode).toBe(200);
    expect(response.body.powerArrays[0].isEquipped).toBe(false);
  });
});
