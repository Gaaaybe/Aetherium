...
describe('Delete Power Array From Character (e2e)', () => {
  ...
  beforeAll(async () => {
    ...
    await request(app.getHttpServer()).post('/users').send({
      name: 'Delete Array User',
      email: 'deletearray@example.com',
      password: '123456',
    });
  ...
  test('[POST] /characters/:id/power-arrays/:powerArrayId/remove', async () => {
    const response = await request(app.getHttpServer())
      .post(`/characters/${characterId}/power-arrays/${arrayId}/remove`)
      .set('Authorization', `Bearer ${accessToken}`)
      .send();

    expect(response.statusCode).toBe(200);
    expect(response.body.powerArrays).toHaveLength(0);
  });
});
