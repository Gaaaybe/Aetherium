      ...
      {characters.length === 0 ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {characters.map((character) => (
            <Card 
              key={character.id} 
              className="group relative h-96 flex flex-col overflow-hidden rounded-2xl border-0 ring-1 ring-gray-200 dark:ring-white/5 cursor-pointer hover:shadow-2xl hover:shadow-purple-500/20 transition-all duration-300"
            ...
            >
              ...
              <div className="absolute inset-0 z-0 bg-black">
                ...
                {/* Overlay Escuro com Gradiente - Protege o texto e dá mood Dark */}
              ...
              <div className="absolute top-4 right-4 z-20 flex gap-2 opacity-100 lg:opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                <Button 
                  variant="destructive" 
                  size="sm" 
                  className="h-8 w-8 p-0 rounded-full bg-red-500/80 hover:bg-red-600 text-white backdrop-blur-sm border border-white/10" 
                  title="Excluir"
                  onClick={(e) => {
                    e.stopPropagation();
              ...
              <div className="relative z-20 mt-auto p-5 md:p-6 flex flex-col justify-end">
                <div className="flex flex-wrap items-center gap-2 mb-2">
                  ...
                  <Badge variant="secondary" className="bg-purple-500/20 hover:bg-purple-500/30 text-purple-200 border-purple-500/30 backdrop-blur-md text-[10px] px-2 py-0">
                ...
                <div className="text-sm text-gray-300 font-medium mb-4 drop-shadow-sm flex items-center gap-1.5 line-clamp-1">
                   <span className="w-1.5 h-1.5 rounded-full shrink-0 bg-purple-400 shadow-[0_0_8px_rgba(168,85,247,0.8)]" />
                   {character.narrative.identity || "Sem Identidade"}
                </div>

                {/* Vitais Flutuantes Neón */}
